...
function readSAMTemplate(): any {
    return yamlParse(readFileSync(path.resolve(process.cwd(), './template.yml')).toString());
}

const print: <T>(_: T) => T = (x) => (console.log(x), x);
        ...
        .map((lambda) => {
            try {
                const loaded = require(path.resolve(
                    process.cwd(),
                    `./src/lambdas/${lambda.CodeUri.split('/').splice(2).join('/')}/${
                        lambda.Handler.split('.')[0]
                    }.ts`
                ));
                ...
                if (l) {
                    const responses =
                        Reflect.getOwnMetadata('lambdi:responses', l.prototype, 'onHandler') || {};
                    const decorators = Reflect.getOwnMetadata(
                        'lambdi:args',
                        l.prototype,
                        'onHandler'
                    );
                    ...
                    const models: Record<string, any> = []
                        ...
                        .map((type: any, i: number) => {
                            switch (
                                (
                                    Object.entries(decorators || []).find(([_, v]) => v === i) || []
                                ).shift()
                            ) {
